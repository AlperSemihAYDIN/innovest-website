      ...
      >
        ...
        <div className="px-2 py-3 border-t border-white/5 space-y-0.5">
          <Link
            href="/"
            target="_blank"
            title={collapsed && !mobileOpen ? 'Siteye Dön' : undefined}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs text-white/30 hover:text-white/60 hover:bg-white/5 transition-all ${collapsed && !mobileOpen ? 'lg:justify-center' : ''}`}
          >
            <ExternalLink size={14} className="flex-shrink-0" />
            <span className={collapsed && !mobileOpen ? 'lg:hidden' : ''}>Siteye Dön</span>
          </Link>
          {user && showLabel && (
          ...
          <button
            onClick={logout}
            title={collapsed && !mobileOpen ? 'Çıkış Yap' : undefined}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs text-red-400/60 hover:text-red-400 hover:bg-red-400/5 transition-all w-full ${collapsed && !mobileOpen ? 'lg:justify-center' : ''}`}
          >
            <LogOut size={14} className="flex-shrink-0" />
            <span className={collapsed && !mobileOpen ? 'lg:hidden' : ''}>Çıkış Yap</span>
          </button>
        </div>
      </aside>
    </>
...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      >
        {/* Logo & Toggle */}
        <div className={`flex items-center border-b border-white/5 h-16 ${collapsed && !mobileOpen ? 'lg:justify-center lg:px-0' : ''} px-5 justify-between`}>
          {showLabel && (
            <h1 className="text-sm font-semibold text-white tracking-wide">
              Innovest <span className="text-[#C1A45D]">Admin</span>
            </h1>
