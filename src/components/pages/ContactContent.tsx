...
import type { ContactPageContent } from '@/lib/pageDefaults';
...
export default function ContactContent({ dict, locale, content }: ContactContentProps) {
  ...
  const formSectionTitle = content?.form?.[l ? 'sectionTitleEn' : 'sectionTitleTr'] ?? (l ? 'Request a ' : 'Danışmanlık ');
  const formSectionHighlight = content?.form?.[l ? 'sectionTitleHighlightEn' : 'sectionTitleHighlightTr'] ?? (l ? 'Consultation' : 'Talep Edin');
  const formSectionSubtitle = content?.form?.[l ? 'sectionSubtitleEn' : 'sectionSubtitleTr'] ?? (l ? 'Complete the form below and a member of our advisory team will be in touch shortly.' : 'Aşağıdaki formu doldurun, danışmanlık ekibimizden biri en kısa sürede sizinle iletişime geçecektir.');
                ...
                <AnimatedSection delay={0.15}>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div>
                        {dcPhones.map((phone) => (
                        <a key={phone} href={`tel:${phone.replace(/[\s+]/g, '')}`} className="flex items-center group" style={{ gap: '16px', marginBottom: '28px' }}>
                          ...
                          <div>
                            <p className="text-base font-medium text-white group-hover:text-gold transition-colors">{phone}</p>
                          </div>
                        </a>
                      ))}
                        <a href={`mailto:${dcEmail}`} className="flex items-center group" style={{ gap: '16px', marginBottom: '28px' }}>
                          <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            <Mail size={18} className="text-gold" />
                          </div>
                          <div>
                            <p className="text-base font-medium text-white group-hover:text-gold transition-colors">{dcEmail}</p>
                          </div>
                        </a>
                        <a href={`https://wa.me/${dcWhatsapp}`} target="_blank" rel="noopener noreferrer" className="flex items-center group" style={{ gap: '16px' }}>
                          <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
