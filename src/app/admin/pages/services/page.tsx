...
import { servicesDefaults, type ServicesPageContent } from '@/lib/pageDefaults';
import {
...
export default function ServicesPageEditor() {
  const slug = 'services';
  const [data, setData] = useState<ServicesPageContent>(servicesDefaults);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setData({ ...servicesDefaults, ...(remote as object) } as ServicesPageContent);
      } catch {} finally { setLoading(false); }
    })();
  }, []);

  function setField<K extends keyof ServicesPageContent>(section: K, value: ServicesPageContent[K]) {
    setData((prev) => ({ ...prev, [section]: value })); setSaved(false);
  }
  ...
  async function handleSave() {
    ...
    } catch (err) { alert('Kaydetme hatası: ' + (err instanceof Error ? err.message : '')); }
    finally { setSaving(false); }
  }
  async function handleReset() {
    if (!confirm('Tüm içerikleri varsayılana döndürmek istediğine emin misin?')) return;
    setData(servicesDefaults); setSaving(true);
    try { await adminApi.updatePage(slug, servicesDefaults as unknown as Record<string, unknown>); setSaved(true); setTimeout(() => setSaved(false), 3000); }
    catch (err) { alert('Hata: ' + (err instanceof Error ? err.message : '')); } finally { setSaving(false); }
  }
  if (loading) return <LoadingSpinner />;

  return (
    <div>
      <StickyEditorHeader title="Hizmetler" description="Hero ve hizmet kartları" previewHref="/services" saving={saving} saved={saved} onSave={handleSave} onReset={handleReset} />
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        ...
        <section className={sectionCard}>
          <div className={sectionInner}>
            <div className="flex items-center justify-between border-b border-white/[0.08] pb-2.5" style={{ marginBottom: '20px' }}>
              ...
              <button onClick={() => setField('services', [...data.services, { titleEn: '', titleTr: '', descEn: '', descTr: '', icon: 'Briefcase', href: '/contact' }])} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/[0.04] hover:bg-white/[0.08] text-[11px] text-white/70 rounded-md uppercase tracking-wider font-medium">
                <Plus size={11} /> Yeni
              </button>
            </div>
              ...
              {data.services.map((s, i) => (
                <div key={i} className={subBlock}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    <Field label="Başlık (TR)" value={s.titleTr} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], titleTr: v }; setField('services', arr); }} />
                    <Field label="Title (EN)" value={s.titleEn} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], titleEn: v }; setField('services', arr); }} />
                    <FieldArea label="Açıklama (TR)" rows={3} value={s.descTr} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], descTr: v }; setField('services', arr); }} />
                    <FieldArea label="Description (EN)" rows={3} value={s.descEn} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], descEn: v }; setField('services', arr); }} />
                    <Field label="İkon (lucide)" placeholder="Building2" value={s.icon} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], icon: v }; setField('services', arr); }} />
                    <Field label="Bağlantı (href)" placeholder="/services" value={s.href} onChange={(v) => { const arr = [...data.services]; arr[i] = { ...arr[i], href: v }; setField('services', arr); }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className={sectionCard}>
          <div className={sectionInner}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
              <Field label="Etiket (TR)" value={data.cta.taglineTr} onChange={(v) => setField('cta', { ...data.cta, taglineTr: v })} />
              <Field label="Tagline (EN)" value={data.cta.taglineEn} onChange={(v) => setField('cta', { ...data.cta, taglineEn: v })} />
